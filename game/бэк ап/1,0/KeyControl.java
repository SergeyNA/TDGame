...
public class KeyControl {
	...
	KeyControl(Image tacticModImage, int startScore, int[][] TTC, int TV, List<Image[]> towerInfoIcon) {
		score = startScore;
		backgroundTacticMod = tacticModImage;
		tacticActive = false;
		enterWaiting = false;
		towerTypeC = false; //типо если башн€ выбрана?
		...
		if (code == 10) {
			if(enterWaiting) {
				if(updateChoice == 0) {
					...
					//!!!!!!!!! вычесть цену апдейта из доступных очков
					...
					updRate = 0;
					towerUpdate = new int[4];
					//вызвать апдейт башни от обновлЄнного массива башненских характеристик 
				}
			}					
		}
		...
		if(showBuyDialog) {
			...
			if(enterWaiting && !message.substring(0, 3).equals("нев")) {
				if(updateChoice == 0) {
					...
					g.drawString(message + towerControl.priceChanges(newTowerDamage, newTowerRadius, newTowerRate, towerTypeCharacteristics[newTowerType][3], false), 450, 250);
					...
					g.drawString(message + towerControl.priceChanges(towerUpdate[0], towerUpdate[1], towerUpdate[2], 0, true), 450, 250);
					//не забыть удалить пометку чтоб вычитать очки за апдейт
