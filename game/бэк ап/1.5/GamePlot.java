...
public class GamePlot {
	...
	GamePlot(int state) {
		...
		dialogBackGround = readImage("images/eventDialog.png");
		...
		case -1:
			...
			route.add(new int[]{100, 320, 330, 50});
			orientation.add(new int[]{1, 0, 1, 2});
			
			route.add(new int[]{100, 285, 335, 35});
			orientation.add(new int[]{1, 2, 1, 2});
			
			route.add(new int[]{70, 160, 25, 100, 175, 180, 165, 315, 330, 50});
			orientation.add(new int[]{3, 2, 1, 2, 3, 0, 3, 2, 1, 2});
			
			//
			
			//остыль детектед! последние координаты принадлежат базе
			...
			towerTypeCharacteristics[4] = new int[]{22, 220, 1, 2, 22, 220, 1, 0, 0, 0};
			//
	...
	/*
	 * мессадж пройгрыша отображать!!
	 * Интерфееееейсыыыыы! нафиг нафиг всё в 1 классе реализовывать. Хотя забвно и работает...
	 * динамическое изменение TTC от времени (подумать надо ли)
	 * возможность оффнуть турельку (аналогично)
	 * боссы в дезматче
	 * конструктор от стринга у эвента
	 * вознаграждение за каждую новую волну в дезматче
	 * сброс числа мобов в дезматче при переходе на новый лвл сложности
	 * изменения сообщения при старте следующей волны в дезматче
	...
	*/
	 ...
	 * дамажить должен снаряд а не башня!
	 ...
	 * * логистика. Возможность сторить там, где сложно.
