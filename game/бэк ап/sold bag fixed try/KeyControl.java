...
public class KeyControl {
	...
	public void getKeyCode(int code, TowerBuilder towerControl, List<TowerUnit> towers) {
		...
		//s
		case 83:
			if(gamerPositionChoice != -1 && towerControl.freeSlot(gamerPositionChoice) && sellChoice == -1) {
		...
		case 8:
			...
			case 10:
				if(enterWaiting) {
					if(sellChoice != -1) {
						score += towerControl.priceChanges(towerInfo[0], towerInfo[1], towerInfo[2], towerTypeCharacteristics[towerInfo[4]][0], towerTypeCharacteristics[towerInfo[4]][1], towerTypeCharacteristics[towerInfo[4]][2], towerInfo[3], towerInfo[4], true) / 2 + 100;
						towers.remove(towerControl.getTower(sellChoice));
						sellChoice = -1;
						enterWaiting = false;
						showBuyDialog = false;
						towerTypeC = false;
